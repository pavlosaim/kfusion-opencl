 ...
 */
//#include "../../include/optimizations.h"
...
//  size.y += 2;
  ...
  // NOTE  - Pavlos //
  // ///// //
  // Padding array, current in[], has more one layer on the perimeter.
  // i.e. Let array F[2][2] get Pad. The output padding array is as follow:
  // [][][][] - Size.x = 2 , Size.y = 2, Loop from 0 to 1
  // []F F [] - [0][0] of F maps to [1][1] now, [0][1] of F maps to [1][2] now
  // []F F [] - [1][0] of F maps to [2][1] now, [1][1] of F maps to [2][2] now
  // [][][][]
  //
  // Adding +1 on both i, j for every position, we actually map the current padding array to the cells of FloatDepth
  // i.e. FloatDepth[0][0] is currently in[1][1](Padding[1][1]), FloatDepth[239][321] is currently in[240][322]
  // This is done whenever we access in[] array.
  // Also every time we access in[] array the size is changed to size.x + 2, size.y + 2
  // 
  // Every other access to any array remains the same, i.e. output[]
  
  // Padding
	const float center = in[(pos.x + 1) + (size.x + 2) * (pos.y + 1)];
  ...
  // Unroll and clamp elimination optimization - Pavlos 
  // Unrolling the baseline loops, visiting all neighbors in radius r
  uint2 curPos;
  float curPix;
  float mod;
  float factor;
  ...
  // i=-1, j=-1
  // curPos = (uint2)(clamp(pos.x - 1, 0u, size.x - 1), clamp(pos.y - 1, 0u, size.y - 1));
  // curPix = in[(curPos.x) + (curPos.y) * (size.x)];
  curPos = (uint2)(pos.x - 1, pos.y - 1);
  curPix = in[(curPos.x + 1) + (curPos.y + 1) * (size.x + 2)];
  ...
  // i=-1, j=0
  // curPos = (uint2)(clamp(pos.x - 1, 0u, size.x - 1), clamp(pos.y + 0, 0u, size.y - 1));
  // curPix = in[(curPos.x) + (curPos.y) * (size.x)];
  curPos = (uint2)(pos.x - 1, pos.y + 0);
  curPix = in[(curPos.x + 1) + (curPos.y + 1) * (size.x + 2)];
  mod = sq(curPix - center);
  factor = gaussian[3];
  t += factor * curPix;
  ...
  // i=0, j=-1
  // curPos = (uint2)(clamp(pos.x + 0, 0u, size.x - 1), clamp(pos.y - 1, 0u, size.y - 1));
  // curPix = in[(curPos.x) + (curPos.y) * (size.x)];
  curPos = (uint2)(pos.x + 0, pos.y - 1);
  curPix = in[(curPos.x + 1) + (curPos.y + 1) * (size.x + 2)];
  mod = sq(curPix - center);
  factor = gaussian[1];
  t += factor * curPix;
  ...
  // i=0, j=0
  // curPos = (uint2)(clamp(pos.x + 0, 0u, size.x - 1), clamp(pos.y + 0, 0u, size.y - 1));
  // curPix = in[(curPos.x) + (curPos.y) * (size.x)];
  curPos = (uint2)(pos.x + 0, pos.y + 0);
  curPix = in[(curPos.x + 1) + (curPos.y + 1) * (size.x + 2)];
  mod = sq(curPix - center);
  factor = gaussian[4];
  t += factor * curPix;
  ...
  //
  // End of Unroll and clamp elimination optimization 
  //
...
//				// const float factor = gaussian[(i + r) * (2*r + 1)] * gaussian[(j + r) + (i + r) * (2*r + 1)];
//			  // const float factor = gaussian[(i + r) * (2*r + 1)] * gaussian[(j + r) + (i + r) * (2*r + 1)] * exp(-mod / (2 * e_d * e_d));
//				// const float factor = gaussian[(i+r + 3*(r+j))] * exp(-mod / (2 * e_d * e_d));
//				const float factor = gaussian[(i+r + 3*(r+j))] ;
//				t += factor * curPix;
//				sum += factor;
//			} else {
//				//std::cerr << "ERROR t float factor = gaussian[i + r] * gaussian[j + r]ILATERAL " <<pos.x+i<< " "<<pos.y+j<< " " <<curPix<<" \n";
//			}
//		}
//	}
...
__kernel void raycastKernel( __global float * pos3D,  //float3
  ...
  pos.x *= 2;
//	const int sizex = get_global_size(0);
	const int sizex = 2 * get_global_size(0);
	...
	if(hit.w > 0.0f ) {
		...
		vstore3(test,pos.x + 1 + sizex * pos.y,pos3D);
		vstore3(test,pos.x + sizex * (pos.y + 1),pos3D);
		vstore3(test,pos.x + 1 + sizex * (pos.y + 1),pos3D);
		...
		if(length(surfNorm) == 0) {
			...
			vstore3((float3)(INVALID,INVALID,INVALID),pos.x + sizex * (pos.y + 1),normal);
			vstore3((float3)(INVALID,INVALID,INVALID),pos.x + 1 + sizex * pos.y,normal);
			vstore3((float3)(INVALID,INVALID,INVALID),pos.x + 1 + sizex * (pos.y + 1),normal);
		} else {
			vstore3(normalize(surfNorm),pos.x + sizex * (pos.y),normal);
			vstore3(normalize(surfNorm),pos.x + 1 + sizex * (pos.y),normal);
			vstore3(normalize(surfNorm),pos.x + sizex * (pos.y + 1),normal);
			vstore3(normalize(surfNorm),pos.x + 1 + sizex * (pos.y + 1),normal);
		}
	} else {
		vstore3((float3)(0),pos.x + sizex * pos.y,pos3D);
		vstore3((float3)(0),pos.x + 1 + sizex * pos.y,pos3D);
		vstore3((float3)(0),pos.x + sizex * (pos.y + 1),pos3D);
		vstore3((float3)(0),pos.x + 1 + sizex * (pos.y + 1),pos3D);
		vstore3((float3)(INVALID, INVALID, INVALID),pos.x + sizex * pos.y,normal);
		vstore3((float3)(INVALID, INVALID, INVALID),pos.x + 1 + sizex * pos.y,normal);
		vstore3((float3)(INVALID, INVALID, INVALID),pos.x + 1 + sizex * (pos.y + 1),normal);
		vstore3((float3)(INVALID, INVALID, INVALID),pos.x + sizex * (pos.y + 1),normal);
...
__kernel void integrateKernel (
  ...
  // Baseline: for(pix.z = 0; pix.z < vol.size.z; ++pix.z, pos += delta, cameraX += cameraDelta) {

// Depending on frame jump.
  uint step = 8;
	for(pix.z = frame % step; pix.z < vol.size.z; pix.z+=step) {

    // Optimized, do not use delta to jump to next pos and cameraX as of in baseline.
    // Instead compute them again
	  pos = Mat4TimeFloat3 (invTrack , posVolume(vol,pix));
	  cameraX = Mat4TimeFloat3 ( K , pos);
    ...
    /* Optimized, following statements removed */
//		if(pixel.x < 0 || pixel.x > depthSize.x || pixel.y < 0 || pixel.y > depthSize.y-1)
//		continue;

		const uint2 px = (uint2)(pixel.x, pixel.y);
//		float depthpx = depth[px.x + (depthSize.x) * (px.y)];
// Add bilateral filter parameters
		float depthpx = depth[px.x + 1 + (depthSize.x+2) * (px.y+1)];

    /* Optimized, following statements removed */
//  if(depthpx == 0) continue;
//  const float diff = ((depthpx) - cameraX.z) * sqrt(1+sq(pos.x/pos.z) + sq(pos.y/pos.z));
  
		const float diff = ((depthpx) - cameraX.z) * (1+(pos.x/pos.z) + sq(pos.y/pos.z));
		if(diff > -mu) {
...
__kernel void mm2metersKernel(
  ...
  // Padding
	depth[(pixel.x + 1) + (depthSize.x + 2) * (pixel.y + 1)] = in[pixel.x * ratio + inSize.x * pixel.y * ratio] / 1000.0f;
}
