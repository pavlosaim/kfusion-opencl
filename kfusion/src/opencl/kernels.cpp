...
void Kfusion::languageSpecificConstructor() {
  ...
  // Padding - Pavlos
	ocl_FloatDepth = clCreateBuffer(context, CL_MEM_READ_WRITE,
			sizeof(float) * (computationSize.x + 2) * (computationSize.y + 2), NULL,
			&clError);
//	ocl_FloatDepth = clCreateBuffer(context, CL_MEM_READ_WRITE,
...
//			&clError);
  ...
  float zeros = 0;
  clEnqueueFillBuffer(commandQueue, ocl_FloatDepth, &zeros, sizeof(zeros), 0, sizeof(float)* (computationSize.x + 2) * (computationSize.y + 2), 0, NULL, NULL);
	checkErr(clError, "clFillBuffer");

  // Added by Pavlos
	ocl_Padd = clCreateBuffer(context, CL_MEM_READ_WRITE, (320+2)*(240+2)*sizeof(float), NULL, &clError);
	checkErr(clError, "clCreateBuffer");
  //
	...
	int x;
  gaussian[0] = 0.9394130111; gaussian[1] = 0.9692332149; gaussian[2] = 0.9394130111; gaussian[3] = 0.9692332149; gaussian[4] = 1; gaussian[5] = 0.9692332149; gaussian[6] = 0.9394130111; gaussian[7] = 0.9692332149; gaussian[8] = 0.9394130111;
...
bool Kfusion::preprocessing(const uint16_t * inputDepth, const uint2 inSize) {
  ...
  // Padding optimization - Pavlos

  // Calculate Time for cpyBuffers - Ignore it
  // start_t = clock();
  //
  //
  // Calculate Time for cpyBuffers
  // end_t = clock();
  // iteration_t = end_t - start_t;
  // time_spent += iteration_t;
  // printf("Time spent: %lf %lf\n", time_spent, time_spent/CLOCKS_PER_SEC);

  // Padding - first and last row
  clError = clEnqueueCopyBuffer(commandQueue, ocl_FloatDepth, ocl_FloatDepth, (computationSize.x + 2)*sizeof(float), sizeof(float),  computationSize.x * sizeof(float), 0, NULL, NULL); 
  clError = clEnqueueCopyBuffer(commandQueue, ocl_FloatDepth, ocl_FloatDepth, ((computationSize.y) * (computationSize.x + 2) + 1) * sizeof(float), ((computationSize.y + 1) * (computationSize.x + 2) + 1) * sizeof(float),  computationSize.x * sizeof(float), 0, NULL, NULL); 

  // Padding all left and right elements
  /*
  int i;
  for (i=1; i < computationSize.y + 1; i++){
    clError = clEnqueueCopyBuffer(commandQueue, ocl_FloatDepth, ocl_FloatDepth, ((i-1) * (computationSize.x + 2) + 1)*sizeof(float), (i * (computationSize.x + 2) + 0)*sizeof(float), sizeof(float), 0, NULL, NULL); 
    clError = clEnqueueCopyBuffer(commandQueue, ocl_FloatDepth, ocl_FloatDepth, (i * (computationSize.x + 0) )*sizeof(float), (i * (computationSize.x + 1))*sizeof(float), sizeof(float), 0, NULL, NULL); 
  }
*/
	arg = 0;
...
//    printf("Level: %d Iterations[level]: %d\n", level, iterations[level]);
...
//  float largestep = 0.1125;

  // Opt - Raycast Every 2 Frames
//	if (frame > 2) {
...
bool Kfusion::integration(float4 k, uint integration_rate, float mu,
	...
	if ((doIntegrate && ((frame % integration_rate) == 0)) || (frame <= 3)) {
		...
		clError = clSetKernelArg(integrate_ocl_kernel, arg++, sizeof(cl_uint),
				(void*) &frame);
		checkErr(clError, "clSetKernelArg12");
